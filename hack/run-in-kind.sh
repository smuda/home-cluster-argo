...
KUBECONFIG=${KUBECONFIG} \
argocd app wait argocd/addon-cert-manager --grpc-web > /dev/null \
  || exit 1
echo ""
...
argocd app wait argocd/addon-cert-manager-approver-policy --grpc-web > /dev/null \
  || exit 1
